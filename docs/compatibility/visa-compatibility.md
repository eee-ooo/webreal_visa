...
| 领域 | 0.5 状态 | 说明 |
|---|---|---|
| C ABI 与 VISA 基础类型 | Implemented (Linux) / NOT_TESTED (Windows 0.4/0.5) | C11/C++20 头编译、固定宽度、平台导出/调用约定；独立历史清单冻结 0.1–0.5 符号所属节点 |
...
| Read/Write | Implemented (Linux/Windows network) | 两平台验证模拟、raw TCP、VXI-11、HiSLIP，Linux 另验证 ASRL、USBTMC 与配置后的 USB RAW bulk/interrupt；支持终止符、EOM/read-ahead、超时、逐 operation 取消和恢复后复用 |
| Clear/Flush | Partial | 模拟、ASRL、VXI-11、HiSLIP、USBTMC class clear 与 RAW 已配置端点 halt/read-ahead discard 支持；raw TCP clear 不伪造消息协议行为；flush 仍按后端能力 |
| 属性 | Partial | 公共超时/终止符/资源/RM/缓冲属性；TCP 地址/主机/端口/Nagle/keepalive；ASRL 波特率/数据位/奇偶/停止位/流控子集 |
| 状态描述 | Partial | 覆盖首批 API 与 0.2–0.5 当前会产生的状态；不是完整 VISA 状态全集 |
| `viTerminate` | Implemented (Linux/Windows network; USB libusb simulator) | `VI_NULL` 取消当前会话排队及进行中操作；VXI-11 使用 abort，HiSLIP 使用 async clear，USBTMC 使用 libusb 异步 cancel callback 后的 bulk abort/clear 回退恢复，RAW 等待 callback 且不提交失败缓冲；异步 job ID 尚无来源 |
| 状态字/触发 | Partial | 模拟、VXI-11、HiSLIP 和 USB488 支持；USB488 覆盖 control/interrupt-IN 状态返回、SRQ/迟到标签过滤和 TRIGGER；raw TCP/ASRL 不伪造协议能力 |
...
| GPIB / 厂商 VISA | Not implemented | `0.5` 范围之外；GPIB 形式的 VXI-11 LAN device name 不等于本机 GPIB 后端 |
| Windows 0.3 原生运行 | Partial | MSVC Debug/Release/ASan、raw TCP、VXI-11/HiSLIP loopback、安装消费和 PE 导出已验证；0.4/0.5 新增 API 与 libusb 适配器尚未原生重跑，Windows ASRL/runtime 和真实仪器 `NOT_TESTED` |
| 二进制兼容承诺 | Partial | 尚未作稳定版 ABI 承诺；ELF 0.1–0.5 节点分层，机器清单禁止既有符号删除/迁移，ELF/PE 继续做精确导出检查 |

标准资源可被成功解析不表示可被打开。`viOpen` 对没有已注册后端或明确未支持模式的合法资源返回 `VI_ERROR_NSUP_OPER`；libusb 存在但设备不存在、权限不足或拔出时分别保留可诊断状态。公共 API 的错误参数/句柄/属性/锁边界以及协议截断和畸形长度已纳入 Linux 与 Windows 无硬件回归；loopback/USB API 模拟器仍只证明本项目客户端与受控规范行为闭环，不等同于第三方真实仪器互操作。Windows 原生网络/协议已有运行证据，但 0.5 libusb 与 ASRL runtime 均不能由其他平台或编译结果推断通过。
