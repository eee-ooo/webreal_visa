...
| 领域 | 0.6 状态 | 说明 |
|---|---|---|
| C ABI 与 VISA 基础类型 | Implemented (Linux) / NOT_TESTED (Windows 0.4–0.6) | C11/C++20 头编译、固定宽度、平台导出/调用约定；新增 40 字节 Prologix v1 配置和 `wrvisaSetPrologixController`，独立历史清单冻结 27 个 0.1–0.6 符号所属节点 |
...
| 本机 GPIB | Partial (Linux controlled endpoints) / NOT_TESTED (real hardware, Windows) | 通用 board/主次地址、EOI/send-end、clear/trigger/serial poll、取消/超时由测试 provider 验证；生产 Prologix Provider 以 RM 版本化配置显式选择串口/TCP，执行 `savecfg 0` 初始化、端点级完整事务仲裁、写转义、有界 EOT 排空和错误后重连，并由 TCP loopback/POSIX PTY 覆盖。它不发现资源、不支持 `INTFC`、别名/跨进程仲裁或任意二进制响应；真实 Prologix/GPIB 仪器和 Windows runtime 为 `NOT_TESTED`。linux-gpib 仍按 ADR-0011 拒绝，NI-488.2 未实现 |
