...
除上述依赖外，当前可构建产物只使用 C/C++ 标准库和操作系统 API。`0.3` 的 XDR、ONC RPC、VXI-11 和 HiSLIP 编解码/状态机，`0.4` 的属性表达式与 alias，`0.5` 的 USBTMC/USB488、USB RAW、provider 和适配逻辑，以及 `0.6` 第一切片的 GPIB provider/transport/session 均依据公开规范或既有项目抽象独立实现，没有复制第三方实现。
