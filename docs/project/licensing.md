...
除上述依赖外，当前可构建产物只使用 C/C++ 标准库和操作系统 API。`0.3` 的 XDR、ONC RPC、VXI-11 和 HiSLIP 编解码/状态机，`0.4` 的属性表达式与 alias，`0.5` 的 USBTMC/USB488、USB RAW、provider 和适配逻辑，以及 `0.6` 的通用 GPIB 与 Prologix 命令层均依据公开规范、官方设备手册或既有项目抽象独立实现，没有复制第三方实现。

## 已评审但拒绝采用的依赖

linux-gpib 4.3.7 来自官方 SourceForge 稳定发布；外层 `linux-gpib-4.3.7.tar.gz` SHA-256 为 `b3ff812606865c85b58b012a4218a6fe3b82d177c8c0abc795cb22db41f3ad68`，其中用户态 `linux-gpib-user-4.3.7.tar.gz` SHA-256 为 `9fadb8a063d7a83c753e428d87ad9677d8fb18ed5a61cb557898ae9b8a987f71`。用户态 `COPYING`、公开头和实现均处于 GPL v2/GPL v2-or-later 边界，没有发现链接例外。

依据本项目第 4 条引入政策，生产库、进程内插件和安装导出目标不得直接链接、延迟链接或通过 `dlopen` 使用 `libgpib`，也不复制其头文件或实现。该依赖不进入 CMake、安装包、许可证目录或 `THIRD_PARTY_NOTICES.md`。详细决策见 [`ADR-0011`](../decisions/0011-linux-gpib-license-boundary.md)，技术证据见 [`0.6 linux-gpib 评审记录`](../progress/2026-08-14-stage-0.6-linux-gpib-review.md)。这是当前工程政策，不是一般性法律意见。
Prologix 第三切片只读取官方公开的 GPIB-USB/GPIB-Ethernet 用户手册并独立实现主机端命令协议；PDF、固件、驱动或厂商库均未进入仓库、构建、安装包或运行时依赖。`Prologix` 是其权利人的产品标识，本项目不宣称附属、认证或背书关系；来源校验值与采用范围见 [`sources.md`](../research/sources.md)。
