...
| `cmake/webreal_visa.map` | ELF 共享库的 0.1/0.2 版本化导出白名单，防止 STL/内部 C++ 符号泄漏为非预期 ABI。 |
| `cmake/webreal_visaConfig.cmake.in` | 安装后 CMake 包入口，恢复线程依赖并导入命名空间目标。 |
| `cmake/uninstall.cmake.in` | 依据 CMake 安装清单生成显式 `uninstall` 目标。 |
| `.github/workflows/build.yml` | 在 Ubuntu 24.04 与 Windows 2025 的 Debug/Release 矩阵中运行同一 CMake/CTest 流程。 |

## 独立消费示例

| 文件 | 职责 |
|---|---|
| `examples/CMakeLists.txt` | 以独立 C 工程通过 `find_package` 消费已安装的静态目标。 |
...
| 文件 | 职责 |
|---|---|
| `include/visa.h` | VISA 兼容基础类型、标准数值、导出/调用约定和 0.2 公共函数；是长期 ABI 边界。 |
| `include/webreal_visa_ext.h` | 项目版本、模拟资源与 ASRL 本机路径映射扩展；使用 `wrvisa*`，不污染 `vi*` 命名空间。 |
...
| 文件 | 职责 |
|---|---|
| `src/api/visa_api.cpp` | 实现全部首批 `vi*` 与项目扩展入口、ABI 异常屏障、句柄/状态映射，并选择模拟、TCP Socket 或 ASRL 后端。 |

## 核心对象与访问控制

| 文件 | 职责 |
|---|---|
| `src/core/backend_session.h` | 后端会话能力接口及默认不支持行为，使 API/核心不依赖具体传输实现。 |
| `src/core/handle_table.h` | 声明对象类型、对象基类和类型化代际句柄表。 |
| `src/core/handle_table.cpp` | 句柄编码、短锁查表、删除、代际递增和耗尽退役实现。 |
| `src/core/objects.h` | 声明 RM、FindList、Session 的所有权、串口映射、属性和 operation 注册接口。 |
| `src/core/objects.cpp` | 实现父子级联关闭、真实后端立即关闭、I/O/属性委派、取消等待和资源清理。 |
| `src/core/lock_manager.h` | 声明按规范化资源隔离的进程内锁协调器。 |
| `src/core/lock_manager.cpp` | 实现排他/共享锁、访问键、嵌套计数、超时和会话释放。 |

## 资源层

| 文件 | 职责 |
|---|---|
| `src/resource/resource_parser.h` | 定义资源类别及包含主机、端口、设备名的传输无关规范化描述。 |
| `src/resource/resource_parser.cpp` | 解析并规范化 ASRL/GPIB/TCPIP/USB 初始标准子集、模拟扩展及括号 IPv6 Socket。 |
| `src/resource/find_expression.h` | 声明可编译、可匹配的 VPP 资源表达式对象。 |
| `src/resource/find_expression.cpp` | 自行解析字符、集合、分组、选择与重复并匹配完整资源名；不暴露宿主正则方言。 |
...
| `src/runtime/operation.cpp` | 实现成功、超时与取消竞争的单一原子提交点，并只触发一次取消 hook。 |
| `src/runtime/io_runtime.h` | 声明进程共享、引用计数持有的有限线程 Asio I/O runtime。 |
| `src/runtime/io_runtime.cpp` | 创建 2–4 个工作线程、work guard，并在最后使用者释放时有序停止。 |
...
| `tests/resource_tests.cpp` | 覆盖标准/扩展资源解析、规范化及 VPP 查找表达式。 |
| `tests/core_tests.cpp` | 覆盖过期/错误类型句柄、一次完成竞争和锁管理器。 |
| `tests/api_tests.cpp` | 覆盖 RM/查找/打开/属性/模拟读写/锁/状态/关闭端到端闭环。 |
